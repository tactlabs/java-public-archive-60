...
public class RockStarProfile {
	...
	public static void main(String[] args) {
		try {
			...
			document.open();
			// Format font = FontFactory.getFont(FontFactory.COURIER, 16,
			// BaseColor.BLACK);
			// Chunk chunk = new Chunk("Hello World");

			document.add(new Paragraph("Personal Bio ", FontFactory.getFont(
					FontFactory.TIMES_BOLD, 15/* Font.BOLD */)));
			document.add(Chunk.NEWLINE);
			document.add(new Paragraph(
					"I'm a full stack developer with a keen interest in strategy and growth. Love creating new things and an enthusiastic learner. Currently, I am working with Vuedata Technologies providing legal Tamil OTT content through website and apps."));
			document.add(Chunk.NEWLINE);
			document.add(Chunk.NEWLINE);
			...
			document.add(Chunk.NEWLINE);
			document.add(new Paragraph("Programming activities", FontFactory
			...
			document.add(Chunk.NEWLINE);
			document.add(new Paragraph("Projects", FontFactory.getFont(
			...
			// document.add(table10);
			document.add(Chunk.NEWLINE);
			document.add(new Paragraph("Employment", FontFactory.getFont(
					FontFactory.TIMES_BOLD, 15)));
			document.add(Chunk.NEWLINE);
			...
			document.add(new Paragraph("TENTKOTTA ", FontFactory.getFont(
					FontFactory.TIMES_BOLD, 15)));
			document.add(Chunk.NEWLINE);
			document.add(new Paragraph(
					"Online movie streaming platform over a variety of devices in high quality legally."));

			List unorderedList = new List(List.UNORDERED);
			unorderedList.add(new ListItem(
					"Understanding the requirements from the client. "));
			unorderedList.add(new ListItem(
					"Interaction with customers to solve their issues. "));
			unorderedList.add(new ListItem(
					"Enhancement of latest features in website. "));
			unorderedList
					.add(new ListItem(
							"Took ownership and integrated the Lyca APIs with the system. "));
			unorderedList
					.add(new ListItem(
							"PayPal API integration and profile management from website and CMS "));
			unorderedList
					.add(new ListItem(
							"Took ownership and enhanced the application for mobile and TV devices."));
			unorderedList.add(new ListItem("Developed the RESTful API"));
			document.add(unorderedList);
			document.add(new Paragraph("Technology Used:", FontFactory.getFont(
					FontFactory.TIMES_BOLD, 15)));
			document.add(new Paragraph(
					"HTML5, CSS, PHP, SQL, XML, JAVA, NODEJS, PAYPAL API, GRAPH API."));
			document.add(Chunk.NEWLINE);
			document.add(new Paragraph("SUPER SARAVANA STORES APP ",
					FontFactory.getFont(FontFactory.TIMES_BOLD, 15)));
			document.add(new Paragraph(
					"An Android Application for Super Saravana Stores, a shopping center for its customer interaction. "));

			List unorderedList1 = new List(List.UNORDERED);
			unorderedList1.add(new ListItem(
					"Understanding the requirements and developed a POC. "));
			unorderedList1
					.add(new ListItem(
							"End to end Development of the Android App from scratch to final product."));
			...
			JFreeChart chart = ChartFactory.createPieChart("strong areas",
					myPiedataset, true, true, false);
			
			document.add((Element) chart);
			
			
			DefaultCategoryDataset dataSet = new DefaultCategoryDataset();
			dataSet.setValue(791, "Population", "Android");
			dataSet.setValue(978, "Population", "PHP");
			dataSet.setValue(1262, "Population", "NodeJs");
			dataSet.setValue(1650, "Population", "FrontEnd(HTML,CSS,Jquery)");
			dataSet.setValue(2519, "Population", "IOS");
			dataSet.setValue(6070, "Population", "SQL");
			dataSet.setValue(6070, "Population", "NOSQL(Mangodb)");
			dataSet.setValue(6070, "Population", "JAVA");
			JFreeChart chart1 = ChartFactory.createBarChart(
					"World Population growth", "Year", "Population in millions",
					dataSet, PlotOrientation.HORIZONTAL, false, true, false);
			document.add((Element) chart1);
	...
	public static PdfPCell getNormalCell(String string, String language,
